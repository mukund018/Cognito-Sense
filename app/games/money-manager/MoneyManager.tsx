...
export default function MoneyManager({ onBack }: { onBack?: () => void } = {}) {
  ...
  const downloadCSV = () => {
    ...
    const mustPaid = logs.filter(l => l.must && l.choice === 'pay' && !l.overBudget).length;
    const mustSkipped = logs.filter(l => l.must && l.choice === 'skip').length;
    const optionalPaid = logs.filter(l => !l.must && l.choice === 'pay' && !l.overBudget).length;
    const optionalSkipped = logs.filter(l => !l.must && l.choice === 'skip').length;
    const overBudgetCount = logs.filter(l => l.overBudget).length;
    const avgRt = logs.length
      ? +(logs.reduce((sum, l) => sum + l.reactionTime, 0) / logs.length).toFixed(2)
      : 0;

    let csvContent = "Money Manager Game Results\n\n";
    csvContent += "Summary\n";
    csvContent += `Budget,₹${budget}\n`;
    csvContent += `Total Spent,₹${spent}\n`;
    csvContent += `Budget Left,₹${budget - spent}\n`;
    csvContent += `Average Reaction Time,${avgRt}s\n\n`;
    
    csvContent += "Decision Summary\n";
    csvContent += `Important Items Total,${mustTotal}\n`;
    csvContent += `Paid Important,${mustPaid}\n`;
    csvContent += `Skipped Important,${mustSkipped}\n`;
    csvContent += `Paid Optional,${optionalPaid}\n`;
    csvContent += `Skipped Optional,${optionalSkipped}\n`;
    csvContent += `Over Budget Attempts,${overBudgetCount}\n\n`;
    
    csvContent += "Detailed Log\n";
    csvContent += "Item,Cost,Priority,Choice,Reaction Time (s),Over Budget\n";
    
    logs.forEach(log => {
      csvContent += `${log.item},₹${log.cost},${log.must ? 'Important' : 'Optional'},${log.choice.toUpperCase()},${log.reactionTime},${log.overBudget ? 'Yes' : 'No'}\n`;
    });
    ...
    link.setAttribute('download', `money_manager_results_${Date.now()}.csv`);
        ...
        <div style={styles.menuContent}>
          ...
          <div style={styles.infoCard}>
            ...
            <ul style={styles.rulesList}>
              ...
              <li>Pay all <span style={{color: '#d32f2f', fontWeight: 'bold'}}>Important</span> bills to win</li>
              <li>Choose wisely with <span style={{color: '#1976d2', fontWeight: 'bold'}}>Optional</span> expenses</li>
  ...
  if (!item) {
    ...
    if (perfectMust && noBudgetErrors && budget - spent <= 1000) {
      performanceMessage = '🏆 Perfect! You paid all important bills and managed your budget wisely!';
    } else if (perfectMust && noBudgetErrors) {
      performanceMessage = '✅ Great job! All important bills paid and no overspending!';
    } else if (mustSkipped > 0) {
      performanceMessage = '⚠️ Warning: You skipped ' + mustSkipped + ' important bill(s)!';
    } else if (overBudgetCount > 0) {
      performanceMessage = '❌ You went over budget ' + overBudgetCount + ' time(s)!';
    }
        ...
        <div style={styles.resultsContent}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.resultRow}>
              ...
              <span style={{...styles.bold, color: budget - spent < 0 ? '#d32f2f' : '#2e7d32'}}>
                ₹{budget - spent}
              </span>
            </div>
            <div style={styles.resultRow}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.resultRow}>
              ...
              <span style={{color: mustPaid === mustTotal ? '#2e7d32' : '#d32f2f', fontWeight: 'bold'}}>
                {mustPaid}
              </span>
            </div>
            <div style={styles.resultRow}>
              <span>✗ Skipped Important:</span>
              <span style={{color: mustSkipped > 0 ? '#d32f2f' : '#666', fontWeight: mustSkipped > 0 ? 'bold' : 'normal'}}>
                {mustSkipped}
              </span>
            </div>
            <div style={styles.resultRow}>
            ...
            <div style={styles.resultRow}>
              ...
              <span style={{color: overBudgetCount > 0 ? '#d32f2f' : '#2e7d32', fontWeight: 'bold'}}>
                {overBudgetCount}
              </span>
            </div>
          </div>
              ...
              <div style={styles.logContainer}>
                {logs.map((l, i) => (
                  <div key={i} style={styles.logRow}>
                    ...
                    <span style={{...styles.logBadge, backgroundColor: l.must ? '#ffebee' : '#e3f2fd', color: l.must ? '#d32f2f' : '#1976d2'}}>
                      {l.must ? 'Important' : 'Optional'}
                    </span>
                    <span style={{...styles.logBadge, backgroundColor: l.choice === 'pay' ? '#e8f5e9' : '#fafafa', color: l.choice === 'pay' ? '#2e7d32' : '#666'}}>
                      {l.choice.toUpperCase()}
                    </span>
                    {l.overBudget && <span style={{...styles.logBadge, backgroundColor: '#ffebee', color: '#d32f2f'}}>Over Budget</span>}
    ...
    <div style={styles.container}>
      ...
      <div style={styles.gameContent}>
        ...
        <div style={styles.itemCard}>
          ...
          <div style={{...styles.priorityBadge, backgroundColor: item.must ? '#ffebee' : '#e3f2fd', color: item.must ? '#d32f2f' : '#1976d2'}}>
        ...
        <div style={styles.buttonRow}>
          ...
          <button style={styles.skipButton} onClick={() => handleDecision(false)}>
            ✗ Skip
          </button>
        </div>
      </div>
